...
class ScapegoatTree(BinarySearchTree):
    ...
    def add(self, x):
        ...
        if d > log32(self.q):
            ...
            w = self.scape_goat_calculo(u)  # Calcula o Bode expiatório
    ...
    def scape_goat_calculo(self, u, tam=1):  # Tamanho inicial 1 pois é o tamanho de uma folha
        ultimo = u
        t = tam
        if u.parent.right == u:
            tp = t + self._size(u.parent.left) + 1
            if 3 * t <= 2 * tp:
                ultimo = self.scape_goat_calculo(u.parent, tp)
        if u.parent.left == u:
            tp = t + self._size(u.parent.right) + 1
            if 3 * t <= 2 * tp:
                ultimo = self.scape_goat_calculo(u.parent, tp)
